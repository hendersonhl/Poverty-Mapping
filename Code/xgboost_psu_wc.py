...
x_census = x.loc[:, x.columns.str.startswith('mun_') |x.columns.str.startswith('HID_') ]
hid = x['HID']
...
for i in range(1,501):
    ...
    X = X.loc[:,  X.columns.str.startswith('mun_') |X.columns.str.startswith('HID_')]  # Set X as chosen predictors
...
prediction.to_csv(outpath + 'gb_census_psu(disaggregated_wc).csv', index = False)
...
# Save results
prediction.to_csv(outpath + 'gb_census_psu_wc.csv')
importance.to_csv(outpath + 'gb_importance_census_psu_wc.csv', index = False)
